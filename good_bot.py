...
def save_media_map(media_map):
    ...
    try:
        ...
        logger.warning("Could not push media map to GitHub in background: %s", e)
        return False

# --- اضافه شده: توابع لیست کاربران ---
def load_users():
...
def save_users(users_list):
    ...
    # ارسال به گیت‌هاب در پس‌زمینه (همانند بقیه فایل‌ها)
    try:
        json_text = json.dumps(users_list, ensure_ascii=False, indent=4)
        success = github_update_file_background(USERS_FILE, json_text, "Update users list via bot")
        if success:
            logger.info("✅ Users list successfully synced to GitHub")
        else:
            logger.warning("⚠️ Failed to sync users list to GitHub")
    except Exception as e:
        logger.warning("Could not push users to GitHub in background: %s", e)
# ------------------------------------

# بارگذاری اولیه تنظیمات، رسانه‌ها و کاربران
CONFIG = load_config()
MEDIA_MAP = load_media_map()
# اضافه شده: بارگذاری اولیه کاربران
...
async def sync_status_command(update: Update, context: CallbackContext) -> None:
    ...
    users_api_url = f"https://api.github.com/repos/{GITHUB_OWNER}/{GITHUB_REPO}/contents/{USERS_FILE}" # اضافه شده
    ...
    try:
        ...
        users_response = requests.get(users_api_url, params={"ref": GITHUB_BRANCH}, headers=headers, timeout=15) # اضافه شده
        ...
        # اضافه شده: بررسی فایل کاربران
        if users_response.status_code == 200:
            users_data = json.loads(base64.b64decode(users_response.json()['content']).decode('utf-8'))
            if json.dumps(users_data, sort_keys=True) == json.dumps(USERS_LIST, sort_keys=True):
                status_text += "✅ فایل users.json همگام است.\n"
            else:
                status_text += "⚠️ فایل users.json با نسخه محلی متفاوت است.\n"
        else:
            status_text += "❌ فایل users.json در گیت‌هاب یافت نشد.\n"
...
# --- اضافه شده: دستور مشاهده لیست کاربران ---
async def users_list_command(update: Update, context: CallbackContext) -> None:
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("این دستور فقط برای مدیر مجاز است.")
        return
    ...
    response_text = f"👥 **آمار کاربران:**\n\n"
    response_text += f"📊 تعداد کل: `{total_users}` نفر\n\n"
    
    if total_users > 0:
        response_text += "🆕 آخرین کاربران اضافه شده:\n"
        # نمایش ۱۰ نفر آخر به ترتیب زمانی (از آخر به اول)
        for uid in USERS_LIST[-10:]:
            response_text += f"• `{uid}`\n"
    else:
        response_text += "هنوز کسی ربات را استارت نزده است."
...
def main() -> None:
    ...
    # دستورات مدیران
    application.add_handler(CommandHandler("syncstatus", sync_status_command))
    application.add_handler(CommandHandler("addadmin", add_admin_command))
    application.add_handler(CommandHandler("removeadmin", remove_admin_command))
    application.add_handler(CommandHandler("listadmins", list_admins_command))
    
    # اضافه شده: دستور لیست کاربران
